/*
 * Frame Comparison Test — platform stubs
 *
 ...
 *
 * Adapted from tests/qemu_harness/harness_stubs.c for native x86_64.
 */
...
/* --- Direct ROM loading (bypass libretro VFS) --- */
void harness_load_rom_direct(const char *path)
{
    FILE *f = fopen(path, "rb");
    if (!f) {
        printf("[compare] ERROR: cannot open ROM '%s'\n", path);
        exit(1);
    }
    ...
    printf("[compare] ROM size: %ld bytes\n", size);
    ...
    printf("[compare] ROM loaded, %u pages mapped\n", rom_blocks);
}
...
void harness_load_bios_direct(const char *path)
{
    FILE *f = fopen(path, "rb");
    if (!f) {
        printf("[compare] WARNING: cannot open BIOS '%s', using open BIOS\n", path);
        return;
    }
    fread(bios_rom, 1, 0x4000, f);
    fclose(f);
    printf("[compare] BIOS loaded from '%s'\n", path);
}
...
/* --- Instruction tracing stubs (not needed for frame comparison) --- */
